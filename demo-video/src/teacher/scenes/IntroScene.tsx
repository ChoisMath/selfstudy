...
// 목차 칩은 문장 1에서 그 목차 이름이 들리는 대략의 위치(비율)에 하나씩 올라온다.
const CHAPTERS: { text: string; at: number }[] = [
  { text: "로그인", at: 0 },
  { text: "출석 체크", at: 0.11 },
  { text: "불참신청 승인", at: 0.22 },
  { text: "감독 교체", at: 0.37 },
  { text: "담임교사 메뉴", at: 0.53 },
];
...
const ChapterChip: React.FC<{ at: number; index: number; text: string }> = ({ at, index, text }) => {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 28, fontWeight: 700, color: colors.gray800, whiteSpace: "nowrap" }}>{text}</span>
    </div>
  );
};
